...
def update_interface(options):
    ...
    try:
        ...
        shutil.copytree(sys.path[0] + "/js", output_dir + "/js")

        if os.path.exists(output_dir + "/jquery-ui"):
            shutil.rmtree(output_dir + "/jquery-ui")
        
        shutil.copytree(sys.path[0] + "/jquery-ui", output_dir + "/jquery-ui")

        if os.path.exists(output_dir + "/" + "styles"):
            shutil.rmtree(output_dir + "/" + "styles")
        shutil.copytree(sys.path[0] + "/styles", output_dir + "/styles")
        
        shutil.copy(sys.path[0] + "/help_30.png", output_dir)
        shutil.copy(sys.path[0] + "/reload.gif", output_dir)
        shutil.copy(sys.path[0] + "/disclaimer.html", output_dir)
...
def create_image(options, contig, contig_statistic, read_status, otype, pair_status):
    ...
    html = open(sys.path[0] + "/zoomtig_template.html").read()

...
function openFastaWindow(title, sequence) {
    top.consoleRef=window.open('','fastaWindow','width=700,height=700,menubar=0,toolbar=0,status=0,scrollbars=1,resizable=1');
...
function fetchReads(event) {
    ...
    var visited = {};
    var win_name = CONTIGO.current_contig_name + '_' + ws + '_' + we;
    var title = 'Reads for ' + CONTIGO.current_contig_name + ' from ' + ws + ' to ' + we;
    top.consoleRef=window.open('', win_name,
...
function openZoomtig(event, dzi, contig_name, contig_length, contig_padded_length, padded_start, padded_end, max_depth, height, ruler_height) {
    window.open(contig_name + '.html', CONTIGO.current_contig_name,
		'width=1000,height=780,menubar=0,toolbar=0,status=1,scrollbars=0,resizable=1');
    return false;
}

function onMouseDown(event) {
    downPx = Seadragon.Utils.getMousePosition(event).minus(
							   Seadragon.Utils.getElementPosition(viewer.elmt));
    
}

function onMouseUp(event) {
    upPx = Seadragon.Utils.getMousePosition(event).minus(
							 Seadragon.Utils.getElementPosition(viewer.elmt));
    
    if ((downPx.x == upPx.x) && (downPx.y == upPx.y)) {
	...
	if (window_size <= 500) {
	    var point = viewer.viewport.pointFromPixel(upPx);
	    var sizePoints = viewer.viewport.getBounds().getSize();
	    var window_size = ((sizePoints.x * CONTIGO.current_contig_assembly_length)/1).toFixed(0);
	    ...
	    var str = 'No read';
	    var title = 'Reads selected from ' + CONTIGO.current_contig_name;
		...
		$.ajax({ url: filename,
				...
				reads.forEach(function (f,n) {
					var rs = parseInt(f.padded_start);
					var re = rs + parseInt(f.padded_length);
					var row = parseInt(f.row);
					if ((bp >= rs) && (bp <= re) && (depth == row)) {
					    res = '>' + f.name + 
						' start=' + rs + 
	    ...
	    alert("Window size must be less than 500 cols to identify reads.");
	}
    }else{

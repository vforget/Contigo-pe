...
function displayFasta(contig_name, consensus){
    ...
    return ">" + contig_name + " length=" + consensus.length + "\n" + fastaSeq;
}

function displaySequence(consensus){
    var fastaSeq = '';
    var i = 0;
    var cend = 0
	for(i = 0; i < consensus.length; i += 50) {
...
function getContigFasta(contig_name) {
    var seq = '';
    var json = $.getJSON('json/' + contig_name + ".json");
...
function loadScaffoldContigs(scaffold_name) {
    var i = 0;
    var txt = '';
    var callback1 = function(){
	alert("still loading...");
    ...
    var contig_names = CONTIGO.scaffolds[scaffold_name].contigs;
    for (var i = 0; i < contig_names.length; i += 1) {
	var contig_name = contig_names[i];
	var my_data = '';
	$.ajax({
		url: 'json/' + contig_name + ".json",
		    dataType: "json",
...
function loadScaffoldSequence(scaffold_name) {
    var i = 0;
    var txt = '';
    var callback1 = function(){
	alert("still loading...");
    ...
    top.consoleRef.document.writeln(
				    '<html><head><title>Assembly Contigs</title></head>'
				    +'<body onLoad="self.focus()" style="background-color: #FFF; color: #000;"><pre style="font-family: Courier New, fixed; font-size: 0.8em;">&gt;' + scaffold_name);
    var contig_names = CONTIGO.scaffolds[scaffold_name].contigs;
    for (var i = 0; i < contig_names.length; i += 1) {
	var contig_name = contig_names[i];
	var my_data = '';
	$.ajax({
		url: 'json/' + contig_name + ".json",
		    ...
		    {
			if (i > 0) {
			    top.consoleRef.document.writeln(displaySequence("N".repeat(CONTIGO.scaffolds[scaffold_name].gaps[i-1])));
			}
			top.consoleRef.document.writeln(displaySequence(data.consensus));
		    }
	    });
    }
    top.consoleRef.document.writeln('</pre></body></html>');
